...
tree structure. Only one process has PPID that is 0, which means this process has no parent process. All the
PIDs will be distinct positive integers.
...
class Solution(object):
    def killProcess(self, pid, ppid, kill):
        ...
        ppid_pid_map = collections.defaultdict(list)
        
        for i in range(len(pid)):
            ppid_pid_map[ppid[i]].append(pid[i])
